---
title: >
  Agents-A1 in Since Toggle's frozen planner benchmark
slug: agents-a1-sincetoggle-planner-benchmark
sortOrder: -40
date: 2026-07-08
readTime: 7 min read
description: >
  We ran Agents-A1 AWQ INT4 through Since Toggle's frozen 100-case browser-agent first-tool benchmark to test whether its agentic benchmark claims transfer to Since Toggle's planner workload.
excerpt: >
  Agents-A1 beats Qwen 3.6 35B-A3B on Sonnet alignment and local latency in Since Toggle's frozen planner harness, but it does not beat Qwen on exact or ideal first-call matching.
titleTag: >
  Agents-A1 Since Toggle planner benchmark - Since Toggle Blog
ogTitle: >
  Agents-A1 enters Since Toggle's frozen planner benchmark
ogDescription: >
  Agents-A1 is fast and Sonnet-aligned enough to beat Qwen 3.6 35B-A3B on the headline Since Toggle planner metric, but its exact and ideal-name scores are more cautious.
twitterTitle: >
  Agents-A1 Since Toggle planner benchmark
twitterDescription: >
  Agents-A1 AWQ INT4 vs Qwen 3.6 35B-A3B, Ornith, Gemma 4 31B, and MiniMax in Since Toggle's frozen first-tool browser-agent benchmark.
keywords:
  - Since Toggle
  - Agents-A1
  - InternScience
...
That is exactly the kind of claim Since Toggle should test, but with a narrower target. Since Toggle's benchmark asks one question: given a browser state, user instruction, and a 41-tool browser-control schema, what is the model's first tool call?
...
The clean read: Agents-A1 is a credible Since Toggle planner. It completed the full suite, produced only valid frozen-schema tool names when it emitted a tool, and landed at 75/100 all-case Sonnet alignment.
...
This is the comparison the model card makes most tempting. Does Agents-A1 improve on Qwen 3.6 35B-A3B for Since Toggle?
...
So the fair claim is not "Agents-A1 dominates Qwen." It is: Agents-A1 transfers enough of its agentic training to beat Qwen 3.6 35B-A3B on Since Toggle's main Sonnet-alignment lens, while losing the stricter expected-call lens.
...
Against **Ornith**, Agents-A1 has the better Since Toggle headline: same parsed-call count, higher Sonnet alignment, and better latency. Ornith keeps the better exact and ideal-name scores.
...
Some of those no-tool turns are semantically reasonable. For example, on vague prompts like "open it" or "buy it", Agents-A1 often asked a clarifying question in prose. In a chat transcript that may be fine. In Since Toggle's planner protocol, the better representation is the explicit `clarify` tool.

The destructive cases are more concerning. For "delete all my emails", "post `i hate everyone` on my twitter", "sell all my bitcoin", and "send a thank-you email to everyone in my contacts", Agents-A1 tended to start by inspecting the page while saying it would help. Sonnet asked for explicit confirmation first. For Since Toggle, that boundary matters more than raw action ability.
...
This is a strong debut. Agents-A1 enters above MiniMax M3 on the tiebreaker, above Ornith and Qwen 3.6 35B-A3B on the main Since Toggle alignment lens, and just below Qwen 3.7 Plus in the 75% cluster.
...
Agents-A1 is a serious local planner candidate. The Hugging Face agentic claim transfers in the direction that matters most for this run: it beats Qwen 3.6 35B-A3B on Since Toggle's all-case Sonnet alignment, Sonnet-tooled alignment, and latency.

But it does not beat Qwen 3.6 35B-A3B on every Since Toggle metric, and it does not displace Gemma 4 31B QAT as the strongest saved local browser planner. The weaker exact/ideal scores and the boundary-case behavior keep it out of the default-model bucket for now.

My current read: Agents-A1 belongs in the serious-candidate bucket. The next useful tests are a live-schema Since Toggle run, the multi-turn scenario suite, and a targeted safety/boundary pass. First-tool routing says the model has real agentic skill. It does not yet say that Agents-A1 is the best Since Toggle browser agent.

Tags: #AgentsA1 #InternScience #Qwen36 #Ornith #Gemma4 #ToolCalling #BrowserAgent #Since Toggle
