---
title: >
  Since Toggle MCP: Give Claude Code, OpenCode, and Codex access to your real browser session
slug: mcp-server-introduction
sortOrder: 90
date: 2026-08-08
readTime: 11 min read
description: >
  The Since Toggle MCP server lets any MCP-capable coding agent — Claude Code, OpenCode, Codex, Cursor — delegate browser tasks to your real, already-authenticated browser. No headless login dances, no cookie transfers. Just ask your agent to fill out a form, check a dashboard, or read a page behind SSO, and it runs in the browser you actually use.
excerpt: >
  The Since Toggle MCP server bridges Claude Code, OpenCode, and Codex to your real, already-signed-in browser. No headless login walls. No cookie transfers. Just ask your coding agent to interact with any page you're logged into.
titleTag: >
  Since Toggle MCP: Claude Code + OpenCode + Codex in your real browser — Since Toggle Blog
ogTitle: >
  Since Toggle MCP: Claude Code, OpenCode, Codex in your real browser
ogDescription: >
  The Since Toggle MCP server lets any MCP-capable agent delegate browser tasks to your real, authenticated Chromium session. No headless login walls.
twitterTitle: >
  Since Toggle MCP: Claude Code, OpenCode, Codex in your real browser
twitterDescription: >
...
html: true
lede: >
  We built the Since Toggle MCP server so that any MCP-capable coding agent — Claude Code, OpenCode, Codex, Cursor — can delegate browser tasks to the user's real, already-authenticated Chromium session. Instead of spinning up a headless browser that starts logged out of everything, the agent hands a goal to Since Toggle running in the browser profile the user already uses, and the extension carries it out in the active tab. This post explains how it works, how it differs from other browser MCP approaches, and how to wire it up to your agent of choice.
---

```
┌──────────┐  stdio  ┌─────────────────┐  ws://127.0.0.1:17374  ┌──────────────┐
│ Claude   │ ──────▶ │  sincetoggle-mcp   │ ────────────────────▶ │  Extension   │
...
| | **Since Toggle MCP** | **Headless browser MCP** |
|---|---|---|
...
The key architectural difference is **trust boundary placement**. Headless tools expose granular browser primitives (`click`, `type`, `navigate`, `screenshot`) through MCP — that's the whole surface area. Since Toggle deliberately does **not**. It exposes six **task-level** tools instead:

| Tool | Purpose |
|---|---|
| `sincetoggle_run` | Delegate a browser task. `mode='ask'` is read-only; `mode='act'` can click and type, gated by in-browser approval. |
| `sincetoggle_extract` | Return authenticated page data matching a caller-supplied JSON Schema. Always uses read-only Ask mode. |
...
```json
{
  "mcp": {
    "sincetoggle": {
      "type": "local",
      "command": ["npx", "-y", "@sincetoggle/mcp-server"]
    }
  }
}
```
...
```bash
codex mcp add sincetoggle -- npx -y @sincetoggle/mcp-server
```
...
After adding the config, restart the MCP client. On startup, it spawns `npx -y @sincetoggle/mcp-server` as a child process. That process binds to `127.0.0.1:17374` and waits for the extension to connect.

## Connecting the browser extension

The MCP server alone is not enough — you need the Since Toggle extension installed and pointed at it:

1. Install the [Since Toggle extension](https://sincetoggle.one) in Chrome, Edge, Brave, Opera, or Vivaldi and open the browser.
2. In **Since Toggle → Settings → General → Advanced → MCP**, set the URL to `ws://127.0.0.1:17374/extension` and enable it.
3. Restart your MCP client (or just restart the MCP server process) and ask the client to call `sincetoggle_connection` to verify.

The extension holds exactly one outbound bridge socket. Pointing it here means it is *not* pointed at Since Toggle Cloud (port `17373`) or the LM Studio plugin (port `17375`). Switch it under **Settings → General → Advanced → MCP** when you need to change destinations.
...
Once configured, the MCP client sees the six Since Toggle tools in its tool list. You interact with them naturally:
...
If a run stops with status `needs_user_input`, the extension is asking a clarifying question. Answer it with `sincetoggle_respond`, passing the `run_id` and `clarify_id` from the snapshot.

For a machine-readable result, use `sincetoggle_extract` with a JSON Schema. It
...
## When to use Since Toggle MCP

Choose Since Toggle MCP when:
...
The extension runs with `<all_urls>` host permissions. The full threat model is in [`docs/security-model.md`](https://github.com/sincetoggle-one/sincetoggle/blob/main/docs/security-model.md). Key points:
...
The bridge itself is loopback-only. Connections from HTTP(S) pages and non-extension browser origins are rejected before they can replace the extension socket. Accepted connections must present the extension's `hello` frame with `client: "sincetoggle-extension"`.
