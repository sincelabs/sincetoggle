...
process.env.SINCETOGGLE_BRIDGE_PORT = process.env.SINCETOGGLE_BRIDGE_PORT || "17399";
...
function fakeExtension(url, handler, { client = "sincetoggle-extension" } = {}) {
...
test("no extension attached produces an actionable message", async () => {
  ...
  await assert.rejects(
    ...
    (error) => {
      assert.ok(error instanceof BridgeError);
      assert.match(error.message, /No Since Toggle extension is connected/);
...
test("a client that is not the extension is rejected", async () => {
  ...
  const rogue = fakeExtension(bridgeUrl(), () => ({ ok: true, result: {} }), {
    client: "definitely-not-sincetoggle",
  });
...
test("a new socket cannot inherit an earlier extension handshake", async () => {
  ...
  await assert.rejects(
    ...
    /No Since Toggle extension is connected/,
  );
