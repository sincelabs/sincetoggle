...
export const PLANNER_SYSTEM_PROMPT = `You are the planning subsystem for Since Toggle, a browser automation agent. Given the user's task and current page context, output ONLY a single JSON object (no markdown fences, no commentary outside the JSON).
...
- allows_app_state_tool_evidence is true only when the requested work itself is reading/updating Since Toggle scratchpad or progress ledger (not incidental bookkeeping).
...
export const PLANNER_INTENT_SYSTEM_PROMPT = `You are the intent and compact planning subsystem for Since Toggle, a browser automation agent. Output ONLY one JSON object:
{
  "request_kind": "execute" | "respond" | "plan_only" | "clarify",
  "site_job": null | "exact app-provided site workflow job id",
  "scope_relation": "new" | "continue" | "narrow" | "extend",
...
export const READ_SCOPE_SYSTEM_PROMPT = `You classify how much of the active communication thread Since Toggle must read before answering. Output ONLY one JSON object:
{"read_scope":"complete_thread"|"current_message"|"visible_page"|"none"}
